// ============================================================
//  js/clientes.js — Autocomplete y búsqueda de clientes
// ============================================================

// ── Autocomplete nueva venta ───────────────────────────────
function autocompletarNombre(val) {
  ...
  lista.innerHTML = matches.slice(0, 5).map(function (v) {
    return '<div class="suggest-item" onclick=\'autorellenarNueva(' + v.id + ')\'>' +
      ...
      '<div style="font-size:11px;color:var(--muted)">' + v.telefono + (v.producto ? ' · ' + v.producto : '') + '</div>' +
    '</div>';
  }).join('');
}
...
function autorellenarNueva(id) {
  ...
  setTimeout(function () { document.getElementById('precio-total')?.focus(); }, 50);
}

// ── Buscador de cliente existente (cobro / aviso / vencida) ─
function buscarClienteExistente(val) {
  var lista = document.getElementById('lista-clientes-existentes');
  if (!lista) return;
  if (!val || val.length < 2) { lista.innerHTML = ''; lista.style.display = 'none'; return; }
  var ventas  = cargarVentas();
  var matches = ventas.filter(function (v) {
    return v.nombre.toLowerCase().includes(val.toLowerCase()) && v.cuotaActual <= v.semanasTotales;
  });
  if (!matches.length) { lista.innerHTML = ''; lista.style.display = 'none'; return; }
  lista.style.display = 'block';
  lista.innerHTML = matches.slice(0, 6).map(function (v) {
    ...
    return '<div class="suggest-item" onclick=\'seleccionarClienteExistente(' + v.id + ')\'>' +
      '<div style="font-weight:600;font-size:13px">' + v.nombre + '</div>' +
      '<div style="font-size:11px;color:var(--muted)">' + v.producto + '</div>' +
      '<div style="display:flex;justify-content:space-between;margin-top:4px">' +
        '<span style="font-size:11px;color:var(--accent)">Cuota ' + v.cuotaActual + '/' + v.semanasTotales + ' · $' + v.cuotaMonto.toLocaleString('es-AR') + '</span>' +
  ...
  if (resumen) {
    ...
    resumen.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">' +
        '<div>' +
          '<div style="font-weight:700;font-size:15px">' + v.nombre + '</div>' +
          '<div style="font-size:12px;color:var(--muted);margin-top:2px">' + v.producto + '</div>' +
          '<div style="font-size:12px;color:var(--muted)">' + v.telefono + '</div>' +
        '</div>' +
        '<div style="text-align:right">' +
          '<div style="font-size:18px;font-weight:800;color:#e8614a">$' + v.cuotaMonto.toLocaleString('es-AR') + '</div>' +
      ...
      '<div style="margin-top:10px;background:var(--border);border-radius:10px;padding:10px;display:flex;justify-content:space-between">' +
        '<div>' +
          ...
          '<div style="font-size:11px;color:var(--muted)">Quedan ' + (v.semanasTotales - v.cuotaActual + 1) + ' por cobrar</div>' +
        '</div>' +
        '<div style="text-align:right">' +
          '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:0.06em">Próx. venc.</div>' +
          '<div style="font-size:14px;font-weight:700;margin-top:2px;color:' + estadoColor + '">' + v.proximoVenc + '</div>' +
        '</div>' +
      '</div>' +
      '<div style="margin-top:8px;background:var(--border);border-radius:99px;height:6px;overflow:hidden">' +
        '<div style="background:linear-gradient(90deg,#e8614a,#f0795f);height:100%;border-radius:99px;width:' +
          Math.round(((v.cuotaActual - 1) / v.semanasTotales) * 100) + '%"></div>' +
      '</div>' +
      '<div style="font-size:10px;color:var(--muted2);text-align:right;margin-top:3px">' +
        Math.round(((v.cuotaActual - 1) / v.semanasTotales) * 100) + '% completado</div>';
  }
