...
function poblarSelectMeses() {
  ...
  ventas.forEach(function (v) {
    if (v.mes_key) meses[v.mes_key] = true;
  });
  ...
  sel.innerHTML = '<option value="">Todos los meses</option>' +
    keys.map(function (k) {
      ...
      return '<option value="' + escapeHtml(k) + '"' + (k === valorActual ? ' selected' : '') + '>' + escapeHtml(label) + '</option>';
    }).join('');
}
...
function renderizarRegistros() {
  ...
  var ventas = cargarVentas()
    .filter(function (v) {
      ...
      var producto = (v.producto || '').toLowerCase();
      if (buscar && !nombre.includes(buscar) && !producto.includes(buscar)) return false;
  ...
  if (!ventas.length) {
    lista.innerHTML = '<div class="venc-empty">Sin registros para estos filtros</div>';
    return;
  }
  ...
  lista.innerHTML = ventas.slice(0, 30).map(function (v) {
    ...
    var progColor = terminado ? 'var(--green)' : 'var(--blue)';
    ...
    return '<div class="reg-item ' + claseEstado + '">' +
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">' +
        '<div style="flex:1;min-width:0">' +
          '<div class="reg-nombre">' + textoSeguro(v.nombre) + '</div>' +
          '<div class="reg-detalle">' + textoSeguro(v.producto, 'Sin producto') + ' · $' + v.precioTotal.toLocaleString('es-AR') + '</div>' +
        '</div>' +
        '<div style="display:flex;align-items:flex-start;gap:10px;flex-shrink:0">' +
          ...
          '</div>' +
          '<button onclick="eliminarRegistro(' + v.id + ')" title="Eliminar" style="background:var(--red-bg);border:1px solid var(--red-br);border-radius:7px;padding:4px 8px;cursor:pointer;font-size:12px;font-weight:700;color:var(--red);transition:all 0.12s;flex-shrink:0">Borrar</button>' +
        '</div>' +
      '</div>' +
      ...
      '<div style="display:flex;justify-content:space-between;margin-top:4px">' +
        '<span style="font-size:10px;color:var(--muted2)">' + textoSeguro(v.fechaVenta) + (v.vendedor ? ' · ' + textoSeguro(v.vendedor) : '') + '</span>' +
  ...
  if (ventas.length > 30) {
    lista.innerHTML += '<div style="text-align:center;padding:10px;font-size:12px;color:var(--muted)">Mostrando 30 de ' + ventas.length + ' registros. Usa el buscador para encontrar los demas.</div>';
  }
}
  ...
  todas.forEach(function (v) {
    (v.historial || []).forEach(function (h) {
      ...
      if (partes.length === 3) {
        ...
        if (mes === hoyMes && anio === hoyAnio) cobrado += Number(h.monto) || v.cuotaMonto || 0;
      }
    });
  });

  var mes = new Date().toLocaleString('es-AR', { month: 'long' });
  el.innerHTML = '<span style="text-transform:capitalize">' + escapeHtml(mes) + '</span>' +
...
function seleccionarDesdeRegistro(id, tipo) {
  ...
  mostrarToast((v ? v.nombre : 'Cliente') + ' seleccionado');
}
